...
export default function AccountDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setAllAccounts(allPlatformsData.map(p => ({ id: p.id, name: p.account, type: p.type_account, balance: p.saldo, isSavings: p.saving, color: `bg-${p.color}-500` })));
  ...
  const [accountTransactions, stats, monthlyData] = useMemo(() => {
    ...
    transactionsForAccount.forEach((transaction) => {
      ...
      if ((transaction.type === "income" && transaction.accountId === accountName) || (transaction.type === "transfer" && transaction.toAccountId === accountName)) {
        monthlyStats[month].income += Math.abs(transaction.amount)
      } else if ((transaction.type === "expense" && transaction.accountId === accountName) || (transaction.type === "transfer" && transaction.accountId === accountName)) {
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              {account.type === "Rekening Bank" ? (<CreditCard className="h-6 w-6 text-muted-foreground" />) : (<Wallet className="h-6 w-6 text-muted-foreground" />)}
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          <Card className="neobrutalism-card">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-semibold">Saldo Saat Ini</CardTitle><Wallet className="h-5 w-5 text-primary" /></CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-primary">{formatCurrency(account.balance)}</div>
              <div className="text-xs text-muted-foreground mt-1">{account.type}{account.isSavings && " • Tabungan"}</div>
            </CardContent>
          </Card>
          <Card className="neobrutalism-card">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-semibold">Total Pemasukan</CardTitle><TrendingUp className="h-5 w-5 text-secondary" /></CardHeader>
            <CardContent>
          ...
          <Card className="neobrutalism-card">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-semibold">Total Pengeluaran</CardTitle><TrendingDown className="h-5 w-5 text-destructive" /></CardHeader>
            <CardContent>
          ...
          <Card className="neobrutalism-card">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-semibold">Total Transaksi</CardTitle><ArrowRightLeft className="h-5 w-5 text-chart-1" /></CardHeader>
            <CardContent>
        ...
        {monthlyData.length > 0 && (
          <Card className="neobrutalism-card mb-8">
            <CardHeader><CardTitle className="text-xl font-bold font-manrope">Aktivitas Bulanan</CardTitle></CardHeader>
            <CardContent>
        ...
        {/* Transaction History */}
        <Card className="neobrutalism-card">
          <CardHeader><CardTitle className="text-xl font-bold font-manrope">Riwayat Transaksi</CardTitle></CardHeader>
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {accountTransactions.map((transaction) => {
                    const toAccount = transaction.toAccountId ? allAccounts.find((acc) => acc.name === transaction.toAccountId) : null
                    const isIncoming = transaction.toAccountId === account.name
                    const isOutgoing = transaction.accountId === account.name && transaction.type !== "income"

                    return (
                      <TableRow key={transaction.id} className="neobrutalism-table-row border-b border-border transition-all duration-75">
                        <TableCell className="font-medium">{new Date(transaction.date).toLocaleDateString("id-ID")}</TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium">{transaction.description}</div>
                            {transaction.subcategory && (<div className="text-xs text-muted-foreground">{transaction.subcategory}</div>)}
                            {toAccount && transaction.type === "transfer" && (
                              <div className="text-xs text-muted-foreground">
                                {isIncoming ? `Dari ${transaction.accountId}` : `Ke ${toAccount.name}`}
                              </div>
                            )}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <Badge className={`${getTransactionBadgeColor(transaction.type)} flex items-center gap-1 w-fit`}>
                            {getTransactionIcon(transaction.type)}
                            {transaction.type === "income" ? "Masuk" : transaction.type === "expense" ? "Keluar" : "Transfer"}
                          </Badge>
                        </TableCell>
                        <TableCell className={`text-right font-semibold ${isIncoming ? "text-secondary" : "text-destructive"}`}>
                          {isIncoming ? "+" : "-"}
